...
<div class="container mx-auto p-4">
    ...
    @endforeach
        ...
        <div class="mb-6 bg-blue-50 border border-blue-100 rounded-lg p-4">
            ...
            @if($assignment->student->name == 'Rest Day')
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-lg">
                        <span class="font-medium">Assignment of {{ \Carbon\Carbon::parse($assignment->cleaning_date)->format('l, F j') }} have checked by {{ $assignment->room->floor->manager->name }} and {{ $assignment->reviews->status }}</span>
                    </p>
                    @elseif ($hasMedia)
                    <p class="text-lg">
                        <span class="font-medium">Assignment of{{ \Carbon\Carbon::parse($assignment->cleaning_date)->format('l, F j') }} {{ $assignment->status }} {{ $assignment->room->floor->manager->name }}</span>
                        to check it.
                    </p>
                    @else
                    <p class="text-lg">
                        <span class="font-medium">Assignment of {{ \Carbon\Carbon::parse($assignment->cleaning_date)->format('l, F j') }} {{ $assignment->status }} {{ $assignment->student->name }}</span>
                        to do it.
                    </p>
                    @endif
                </div>
